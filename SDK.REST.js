...
SDK.REST = {
 ...
 searchanoRecord: function (ano, type, select, expand, successCallback, errorCallback) {
    if (type === null) {
		type = 'Contact'
  }
  ///<summary>
  ...
  ///</summary>
  ///<param name="ano" type="String">
  /// A String representing the search value for the 'default' contact record to retrieve.
  ///</param>
  ///<param name="type" type="String">
  /// The Schema Name of the Entity type record to retrieve.
  /// For an Account record, use "Account" default is Contact
  ///</param>
  ...
  /// This function must accept an Error object as a parameter.
  /// </param>
 /// this._stringParameterCheck(ano, "SDK.REST.searchanoRecord requires the ano parameter is a string.");
  this._stringParameterCheck(type, "SDK.REST.searchanoRecord requires the type parameter is a string.");
  if (select != null)
   this._stringParameterCheck(select, "SDK.REST.searchanoRecord requires the select parameter is a string.");
  if (expand != null)
   this._stringParameterCheck(expand, "SDK.REST.searchanoRecord requires the expand parameter is a string.");
  this._callbackParameterCheck(successCallback, "SDK.REST.searchanoRecord requires the successCallback parameter is a function.");
  this._callbackParameterCheck(errorCallback, "SDK.REST.searchanoRecord requires the errorCallback parameter is a function.");

  var systemQueryOptions = "$filter=substringof(\'"+ano+"\',MobilePhone)";
  ...
  if (select === null) {
      selectString = "$select=ContactId"
	  systemQueryOptions = systemQueryOptions + selectString;
    }
  

  var req = new XMLHttpRequest();
  req.open("GET", encodeURI(this._ODataPath() + type + "Set?" + systemQueryOptions), true);
  req.setRequestHeader("Accept", "application/json");
  req.setRequestHeader("Content-Type", "application/json; charset=utf-8");
  req.onreadystatechange = function () {
   if (this.readyState == 4 /* complete */) {
